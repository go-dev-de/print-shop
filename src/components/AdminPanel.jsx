...
export default function AdminPanel() {
  ...
  const loadReviews = useCallback(async (page = 1, append = false) => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/reviews?page=${page}&limit=10&admin=true`, { cache: 'no-store' });
      const data = await res.json();
      ...
      setReviewsHasMore(newReviews.length === 10); // Если получили меньше 10, значит это последняя страница
      setReviewsPage(page);
    } catch (e) {
      setError(String(e.message || e));
    } finally {
  ...
  }, [reviewsLoading, reviewsHasMore, reviewsPage, loadReviews]);

  // Callback для Intersection Observer
  const lastReviewElementRef = useCallback(node => {
    if (reviewsLoading) return;
    if (reviewsObserverRef.current) reviewsObserverRef.current.disconnect();
    reviewsObserverRef.current = new IntersectionObserver(entries => {
      if (entries[0].isIntersecting && reviewsHasMore) {
        loadMoreReviews();
      }
    });
    if (node) reviewsObserverRef.current.observe(node);
  }, [reviewsLoading, reviewsHasMore, loadMoreReviews]);
  ...
  const deleteUser = async (userId) => {
    ...
    // Если пользователь не найден в локальном состоянии, просто продолжаем
    // Сервер сам проверит существование пользователя
    if (user?.role === 'admin') {
      alert('Нельзя удалить администратора');
      return;
    }
    
    const userName = user?.name || user?.email || `пользователя с ID ${userId.slice(0, 8)}...`;
    ...
    try {
      ...
      console.log('📊 Текущие пользователи в состоянии:', users.length);
      ...
      // Обновляем список пользователей из ответа сервера
      if (data.users) {
        setUsers(data.users);
      } else {
        // Fallback: перезагружаем список с сервера
        await loadUsers();
      }
      ...
      alert(String(e.message || e));
      // При ошибке обновляем список пользователей
      try {
        await loadUsers();
      } catch (loadError) {
        console.error('❌ Ошибка перезагрузки пользователей:', loadError);
      }
    }
  };
          ...
          <table className="min-w-full text-sm border-separate border-spacing-0">
            <thead>
              <tr>
                ...
                <th className="p-3 bg-gray-900 text-white font-semibold text-left">Роль</th>
            ...
            <tbody>
              ...
              {users.length === 0 && (
                <tr>
                  <td className="p-4 text-center text-gray-900 font-semibold" colSpan={5}>Нет пользователей</td>
                </tr>
              )}
            </tbody>
          </table>
            ...
            <div className="space-y-4">
                ...
                >
                  ...
                  <div className="mb-3">
                    ...
                    <p className="text-gray-700 text-sm leading-relaxed">
                      {review.content || 'Нет текста отзыва'}
                    </p>
                  </div>
                  ...
                  <div className="flex justify-between items-center text-xs text-gray-500">
                    <div className="flex space-x-4">
                      <span>
                        <span className="font-medium">Автор:</span> {review.author_name || 'Анонимный'}
                      </span>
                      {review.author_email && (
                        <span>
                          <span className="font-medium">Email:</span> {review.author_email}
                        </span>
                      )}
                    </div>
                    <span>
                      {review.created_at ? new Date(review.created_at).toLocaleDateString('ru-RU') : 'Неизвестная дата'}
                    </span>
                  </div>
              ...
              {reviewsLoading && reviews.length > 0 && (
                <div className="text-center py-4">
                  ...
                  <p className="text-gray-600 mt-2 text-sm">Загружаем еще отзывы...</p>
                </div>
              )}
