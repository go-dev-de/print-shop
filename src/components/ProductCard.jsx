...
export default function ProductCard({ product }) {
  ...
  const handleImageLoad = (imageIndex) => {
    setImagesLoaded(prev => new Set(prev).add(imageIndex));
  };
        ...
        {allImages.length > 0 ? (
          <>
            <Image 
              ...
              width={300}
              height={256}
              priority={currentImageIndex === 0} // Приоритет только для первого изображения
              loading={currentImageIndex === 0 ? 'eager' : 'lazy'} // Lazy loading для остальных
            ...
            {!imagesLoaded.has(currentImageIndex) && (
              <div className="absolute inset-0 bg-gray-200 animate-pulse flex items-center justify-center">
                <div className="w-16 h-16 bg-gray-300 rounded-full animate-ping"></div>
              </div>
            )}
